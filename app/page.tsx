...
export default function Home() {
  ...
  const game = snapshot?.game; const myTurn = !!game && game.phase === "playing" && game.turn === session?.seat; const myExchange = !!game && game.phase === "exchange" && game.turn === session?.seat; const canSelect = myTurn || myExchange;
  ...
  return <><main className="game-shell">
    ...
    <section className="table">
      ...
      <div className="table-center"><p className="turn-label">{game?.phase === "playing" ? myTurn ? t.yourTurn : `${t.turnOf} ${game?.players[game.turn]?.name}` : game?.phase === "exchange" ? myExchange ? t.exchangeYourTurn : `${t.exchangeWaiting} ${game?.players[game.turn]?.name}` : t.matchOver}</p><div className="last-play">{game?.pile?.length ? <><small>{game.currentPlay ? `${t.kinds[game.currentPlay.kind]} · ${game.players[game.currentPlay.seat].name}` : t.closedRound}</small><div className="pile-stack">{(game.pile ?? []).slice(-7).map((play,index) => <div className="pile-play" key={`${play.seat}-${index}-${play.cards[0]?.id}`} style={{ transform:`translate(${(index%3-1)*5}px, ${index*2}px) rotate(${(index%5-2)*2.4}deg)`, zIndex:index+1 }}>{play.cards.map((card) => <CardView card={card} copy={t} small key={card.id} />)}</div>)}</div>{!game.currentPlay && <p className="free-lead">{t.newRound} · {t.freePlay}</p>}</> : <div className="empty-play"><span>♠</span><p>{t.newRound}<br /><small>{t.freePlay}</small></p></div>}</div>{game?.passes.length ? <p className="pass-line">{t.passed}: {game.passes.map((s) => game.players[s].name).join(", ")}</p> : null}</div>
    </section>
