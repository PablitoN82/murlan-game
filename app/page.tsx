...
export default function Home() {
  ...
  useEffect(() => { const savedLang = localStorage.getItem("murlan-language") as Language | null; if (savedLang && translations[savedLang]) setLang(savedLang); const raw = localStorage.getItem(STORE); const query = new URLSearchParams(location.search).get("room"); if (query) setCode(query.toUpperCase()); if (raw) { try { const active = JSON.parse(raw) as Session; saveSession(active); refresh(active); } catch { localStorage.removeItem(STORE); } } }, [refresh]);
  ...
  if (!session || !snapshot) return <><main className="landing">
    ...
    <section className="hero">
      ...
      <div className="entry-card"><div className="entry-tabs"><span>{t.newGame}</span><span>{t.join}</span></div><label>{t.yourName}<input value={name} onChange={(e) => setName(e.target.value)} maxLength={24} placeholder={t.namePh} /></label><label>{t.howMany}<div className="people-picker">{[1,2,3,4].map((n) => <button key={n} type="button" onClick={() => setHumans(n)} className={humans === n ? "active" : ""}><b>{n}</b><small>{n === 1 ? t.youBots : n === 4 ? t.allHuman : `${n} ${t.humans}`}</small></button>)}</div></label><button className="primary" disabled={busy || !name.trim()} onClick={() => enter("create")}>{t.create}</button><div className="or"><span>{t.orCode}</span></div><div className="join-row"><input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} maxLength={6} placeholder={t.code} /><button disabled={busy || !name.trim() || code.length < 4} onClick={() => enter("join")}>{t.join}</button></div>{error && <p className="error">{error}</p>}<p className="microcopy">{t.noDownload}</p></div>
  ...
  </main><Lobby name={session.name} roomCode={session.code} copy={t} lang={lang} onJoin={() => {}} /></>;
}
...
function Lobby({ name, roomCode, copy, lang, onJoin }: { name: string; roomCode?: string; copy: Copy; lang: Language; onJoin: (code: string) => void }) {
  ...
  async function translateMessage(message: LobbyMessage) { setTranslating(message.id); try { const res = await fetch("/api/translate", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ text:message.text, target:lang }) }); const data = await res.json() as { translation?: string }; if (data.translation) setTranslated((old) => ({ ...old, [message.id]: data.translation! })); } finally { setTranslating(null); } }
